...
# HM4 -  новый Feature View
driver_activity_stats_fv = FeatureView(
...
# HM4 -  1 on-demand Feature View
@on_demand_feature_view(
    sources=[driver_quality_fv, driver_activity_stats_fv],
    schema=[
        Field(name="efficiency_score", dtype=Float64),
        Field(name="quality_index", dtype=Float64),
    ],
)
def realtime_driver_metrics(inputs: pd.DataFrame) -> pd.DataFrame:
    df = pd.DataFrame()

    conv = inputs["conv_rate"].fillna(0)
    acc = inputs["acc_rate"].fillna(0)
    trips = inputs["avg_daily_trips"].fillna(0)

    df["quality_index"] = conv * 0.7 + acc * 0.3

    df["efficiency_score"] = df["quality_index"] * np.log1p(trips)

    return df


driver_activity_v5 = FeatureService(
    name="driver_activity_v5",
    features=[
        driver_quality_fv,
        driver_activity_stats_fv,
        realtime_driver_metrics,
    ],
)